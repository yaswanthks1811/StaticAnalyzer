...
public class PEResourceAnalyzer implements Serializable {
    ...
    private void parseResourceEntry(int entryOffset, int dataOffset, int level, String path) {
        if ((dataOffset & 0x80000000) != 0) {
            ...
            if (parts.length >= 2) {
                ...
                try {
                    int typeId = Integer.parseInt(parts[1].replace("ID: ", ""));
                    entry.type = getResourceTypeName(typeId);
                } catch (NumberFormatException e) {
                    // Not a number, treat as custom name
                    entry.type = parts[1];
                }
    ...
    private void analyzeResourceContent(ResourceEntry entry) {
        ...
        // Check for common image formats regardless of resource type name
        if (header.length >= 8) {
            if (header[0] == (byte)0x89 && "PNG".equals(new String(header, 1, 3))) {
                entry.details = "PNG image";
                return;
            } else if ("GIF8".equals(new String(header, 0, 4))) {
                entry.details = "GIF image";
                return;
            } else if (header[0] == (byte)0xFF && header[1] == (byte)0xD8) {
                entry.details = "JPEG image";
                return;
            } else if ("BM".equals(new String(header, 0, 2))) {
                entry.details = "BMP image";
                return;
            }
        }
        ...
        if (entry.type.equals("RT_ICON") || entry.type.equals("RT_GROUP_ICON")) {
            ...
            entry.details = "Version info";
        }
        else if (entry.type.equals("RT_DIALOG")) {
            entry.details = "Dialog template";
        }
        else if (entry.type.equalsIgnoreCase("IMAGE") || entry.type.equalsIgnoreCase("PNG")) {
            entry.details = "Custom image resource";
        }
        else {
            entry.details = "";
        }
